...
class Trainer:
    ...
    def setup_simulation(self, dataset_dir, grid_size=100):
        ...
        if downsample_scale > 0 and downsample_scale < 1.0:
            ...
            sim_xyzs = downsample_with_kmeans_gpu(sim_xyzs, num_cluster).to(torch_dtype)
    ...
    def add_constant_force(self, center_point, radius, force, dt, start_time, end_time, impulse_mode="grid"):
        ...
        if pos_array is None or save_name == "debug":
            ...
            for i in tqdm(range(int((30) * num_sec))):
                ...
                for substep_local in range(num_substeps):
                    ...
                    # if i < 1 and substep_local % 10 == 0:
                    #     # save the particle positions for the first 10 substeps
                    #     paricle_pos = wp.to_torch(next_state.particle_x).detach().clone().cpu().numpy()
                    #     np.save(pos_path.replace(".npy", f"_substep_{substep_local}.npy"), paricle_pos)
                    # if i == 1:
                    #     pdb.set_trace()
